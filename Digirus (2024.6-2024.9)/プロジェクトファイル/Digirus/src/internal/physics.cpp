...
void CPhysics::Update()
{
	...
	for (int i = 0; i < pCollisions.size(); i++)
	{
		// リジッドボディを取得する
		...
		if (pRigidBody != nullptr)
		{
			...
			// 向きの反映
			pTransform->SetRot(QuaternionToEuler(D3DXQUATERNION(resultTrans.getRotation().getX(), resultTrans.getRotation().getY(), resultTrans.getRotation().getZ(), resultTrans.getRotation().getW())));
		}
	}
...
void CCollision::Uninit()
{
	...
	{
		CManager::GetPhysics()->GetDynamicsWorld().removeCollisionObject(m_pGhostObject);
	...
	if (m_pRigidBody != nullptr)
	{
		...
		CManager::GetPhysics()->GetDynamicsWorld().removeRigidBody(m_pRigidBody);
		delete m_pRigidBody;
		m_pRigidBody = nullptr;
	}
}
...
{
	// 物理ワールドを取得する
	btDiscreteDynamicsWorld& pDynamicsWorld = CManager::GetPhysics()->GetDynamicsWorld();
...
void CCollision::UpdateShapes()
{
	...
	for (int i = 0; i < pColliders.size(); i++)
	{
...
void CActionInterface::updateAction(btCollisionWorld* collisionWorld, btScalar deltaTimeStep)
{
	...
	for (int nCntObj = 0; nCntObj < pCollisions.size(); nCntObj++)
	{
		...
		{ // ゴーストオブジェクトかつトリガーのとき
			...
			// Exitイベント用処理
			try {
				for (int i = 0; i < pOverlappingVector.size(); i++)
				{
					...
					if (!bCurrentExist)
					{ // 前回はあって、今回は存在しないとき
						for (int nCntComp = 0; nCntComp < pComponents.size(); nCntComp++)
						{
			...
			{
				std::cout << "Exit Event Error" << std::endl;
			}
			...
			try {
				...
				for (int i = 0; i < nNumOverlapping; i++)
				{
					...
					if (pCurrentOverlappingGameObj != nullptr)
					{
						...
						for (int nCntOverlapping = 0; nCntOverlapping < pOverlappingVector.size(); nCntOverlapping++)
						{
			...
			{
				std::cout << "Stay and Enter Events Error" << std::endl;
			}
			...
			{
				std::cout << "Update Overlapping Objects Error" << std::endl;
			}
		}
	}
}