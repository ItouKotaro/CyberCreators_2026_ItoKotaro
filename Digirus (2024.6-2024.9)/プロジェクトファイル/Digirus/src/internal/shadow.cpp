...
void CShadow::Draw(CCamera* pCamera)
{
    ...
    // 影の描画
    GameObject* pMainCamera = GameObject::Find("Camera");
    CNextPortal* pNextPortal = pMainCamera->GetComponent<CCameraMove>()->GetNextPortal();

    if ((pNextPortal == nullptr && pCamera == pMainCamera->GetComponent<CCamera>()) ||
        (pNextPortal != nullptr && pCamera == pNextPortal->GetNextCamera()->GetComponent<CCamera>())
    )
    { // 軽量化のため、メインカメラのみ
        DrawShadow(pCamera);
    }
...
void CShadow::DrawShadow(CCamera* pCamera)
{
    ...
    for (int nCntLight = 0; nCntLight < pLightList.size(); nCntLight++)
    {
        // ライトの取得
        ...
        for (int nCntModel = 0; nCntModel < pMeshList.size(); nCntModel++)
        {
            // モデルを取得する
