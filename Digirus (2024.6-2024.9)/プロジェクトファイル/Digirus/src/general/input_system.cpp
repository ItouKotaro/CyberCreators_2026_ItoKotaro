...
void CInputSystem::Update()
{
	// すべての入力デバイスを更新する
	vector<CInputDevice*>& pInputDevice = CInputDevice::GetInputDevice();
	for (int i = 0; i < pInputDevice.size(); i++)
	{
	...
	for (int i = 0; i < m_keyData.size(); i++)
	{
		...
		{ // キー名が合致するとき
			...
			case INPUT_TYPE::PRESS:
				for (int nCntKey = 0; nCntKey < m_keyData[i].key.size(); nCntKey++)
				{
...
bool CInputSystem::onPress(const std::string& key)
{
	...
	vector<CInputDevice*>& pInputDevice = CInputDevice::GetInputDevice();
	for (int nCntKey = 0; nCntKey < sKeys.size(); nCntKey++)
	{
...
HRESULT CInputSystem::LoadFile(const std::string& sPath)
{
	...
	if (ifs.fail())
	{ // ファイルの読み込みに失敗
		MessageBox(CManager::GetHWND(), "入力情報ファイルの読み込みに失敗しました", "エラー", MB_OK); //終了メッセージ
		return E_FAIL;
	}
...
{
	// デバイス一覧から削除する
	for (int i = 0; i < m_apInputDevice.size(); i++)
	{
