...
HRESULT CMesh::LoadMeshX(std::string sPath, bool bShadow)
{
	...
	// メッシュを読み込む
	CDataMesh* pMeshData = CManager::GetDataManager()->RefMesh(sPath);
	...
	for (int nCntMat = 0; nCntMat < (int)m_dwNumMat; nCntMat++)
	{
		...
		{ // テクスチャがあるとき
			...
			// テクスチャを作成
			m_apTexture[nCntMat] = CManager::GetDataManager()->RefTexture(pMat[nCntMat].pTextureFilename)->GetTexture();
		}
		else
		{
			m_apTexture[nCntMat] = nullptr;
