...
void CMovementSpace::Update()
{
		...
		INPUT_INSTANCE->onPress("d"))
	{
		...
		transform->Translate(cosf(fAngle) * m_fSpeed, 0.0f, sinf(fAngle) * m_fSpeed);
	}
...
float CMovementSpace::GetMoveAngle()
{
	if (INPUT_INSTANCE->onPress("w"))
	{
		...
		{
			return (D3DX_PI / 4 + transform->GetRot().y);
		}
	...
	else if (INPUT_INSTANCE->onPress("s"))
	{
		if (INPUT_INSTANCE->onPress("a"))
		{
			return ((D3DX_PI / 4) * 3 + transform->GetRot().y);
		}
		else if (INPUT_INSTANCE->onPress("d"))
		{
			return ((-D3DX_PI / 4) * 3 + transform->GetRot().y);
		...
		{
			return (-D3DX_PI + transform->GetRot().y);
		}
	}
	else if (INPUT_INSTANCE->onPress("a"))
	{
		return (D3DX_PI / 2 + transform->GetRot().y);
	}
	else if (INPUT_INSTANCE->onPress("d"))
	{
		return (-D3DX_PI / 2 + transform->GetRot().y);
