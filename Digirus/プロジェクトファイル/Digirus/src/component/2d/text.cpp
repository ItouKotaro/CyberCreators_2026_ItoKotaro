...
const CText::FontData CText::LoadFonts[] = {
    {"data\\FONT\\BestTen-CRT.otf", "ベストテン-CRT"},
    {"data\\FONT\\JF-Dot-MPlus12.ttf", "JFドットM+12"},
...
{
    // スプライトを作成する
    D3DXCreateSprite(CManager::GetRenderer()->GetDevice(), &m_pSprite);
    ...
    m_outlineSize = 0;
    m_fMaxHeight = 0.0f;
    m_edgeColor = D3DXCOLOR(1.0f, 1.0f, 1.0f, 1.0f);
...
// [CText] アウトラインの色設定
//=============================================================
void CText::SetEdgeColor(const D3DXCOLOR& color)
{
...
void CText::UpdateText()
{
    ...
    std::smatch match;
    auto it = m_text.cbegin();
    while (std::regex_search(it, m_text.cend(), match, sizeTag))
    {
        CSizeTag* pSizeTag = new CSizeTag();
        pSizeTag->SetSize(atoi(match.str(1).c_str()));

        pSizeTag->SetIdx(match.position(0));
        m_text.erase(match.position(0), match.length());
    ...
    it = m_text.cbegin();
    while (std::regex_search(it, m_text.cend(), match, colorTag))
    {
    ...
    it = m_text.cbegin();
    while (std::regex_search(it, m_text.cend(), match, colorResetTag))
    {
        CColorTag* pColorTag = new CColorTag();
        pColorTag->SetFillColor(m_fillColor);
        pColorTag->SetEdgeColor(m_edgeColor);
    ...
    it = m_text.cbegin();
    while (std::regex_search(it, m_text.cend(), match, fontColorTag))
    {
    ...
    it = m_text.cbegin();
    while (std::regex_search(it, m_text.cend(), match, fontColorResetTag))
    {
        CFontColorTag* pFontColorTag = new CFontColorTag();
    ...
    it = m_text.cbegin();
    while (std::regex_search(it, m_text.cend(), match, edgeColorTag))
    {
    ...
    it = m_text.cbegin();
    while (std::regex_search(it, m_text.cend(), match, edgeColorResetTag))
    {
        CEdgeColorTag* pEdgeColorTag = new CEdgeColorTag();
        pEdgeColorTag->SetColor(m_edgeColor);
    ...
    int nTextLength = m_text.length();     // 文字数
    float fTextWidth = 0.0f;
    int fontSize = m_fontSize;
    D3DXCOLOR fillColor = m_fillColor;
    D3DXCOLOR edgeColor = m_edgeColor;
    ...
    RECT* info)
{
    ...
    // ハンドルとデバイスの取得
    HWND hWnd = CManager::GetRenderer()->GetHWND();
