...
public:
	...
	void SetQuaternion(D3DXQUATERNION quaternion) {m_rotation = QuaternionToYPR(quaternion);}
...
private:
	...
	// クォータニオンからオイラー角
	D3DXVECTOR3 QuaternionToYPR(D3DXQUATERNION q)
	{
		Eigen::Quaternionf quaternion(q.w, q.x, q.y, q.z);
		auto euler = quaternion.toRotationMatrix().eulerAngles(1, 0, 2);
		return D3DXVECTOR3(euler.y(), euler.x(), euler.z());
	}
};
