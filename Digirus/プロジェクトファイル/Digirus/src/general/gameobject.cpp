...
GameObject* GameObject::LoadPrefab(const std::string& sPath, Transform transform)
{
	...
	{ // プレハブが含まれているとき（必須）
		...
		{ // コリジョン情報が含まれているとき
			...
			// コリジョン情報
			for (int i = 0; i < jPrefab["prefab"]["collision"].size(); i++)
			{
				// コリジョンのタイプを取得する
	...
	{ // オブジェクトが含まれているとき
		for (int i = 0; i < jPrefab["object"].size(); i++)
		{
			...
			{ // オブジェクトにコリジョン情報が含まれているか
				...
				// コリジョン情報
				for (int nCntCol = 0; nCntCol < jCol.size(); nCntCol++)
				{
					// コリジョンのタイプを取得する
	...
	{ // 親子情報が含まれているとき
		for (int i = 0; i < jPrefab["parent"].size(); i++)
		{
			// 親の設定
